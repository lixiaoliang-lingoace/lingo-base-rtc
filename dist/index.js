    ...
    try {
      ...
      const promiseList = checkList.map(item => {
        return new Promise(async function (resolve) {
          ...
          if (kindList.length > 0 && currentDevice.deviceId && !kindList.some(p => p.deviceId === currentDevice.deviceId)) {
            // 当前的设备id已不可用，重新指定一个
...
export { LingoBaseRTC, LingoBaseRTCClient, LingoBaseRemoteTrack, LingoBaseStore, LingoBaseTrack, LingoRTCError, LingoRTCErrorCode, RTCConnectionState, config };
